# Current composed VP8L symbolic Callgrind profile
...
```text
42,221,393 (100.0%)  PROGRAM TOTALS
10,200,808 (24.16%)  src/lossless/decoder/mod.rs:<image_webp::lossless::decoder::LosslessDecoder<core::io::util::Take<&mut core::io::cursor::Cursor<alloc::vec::Vec<u8>>>>>::decode_image_stream [/home/runner/work/image-webp/image-webp/target/release/examples/profile_vp8l_current]
-- Auto-annotated source: src/lossless/decoder/mod.rs
    1,116 ( 0.00%)              Self::Implicit(code_lengths) => HuffmanTree::build_implicit(code_lengths),
1,439,186 ( 3.41%)  => /home/runner/work/image-webp/image-webp/src/lossless/decoder/huffman.rs:<image_webp::lossless::decoder::huffman::HuffmanTree<9>>::build_implicit (124x)
...
26,831,058 (63.55%)  => src/lossless/decoder/mod.rs:<image_webp::lossless::decoder::LosslessDecoder<core::io::util::Take<&mut core::io::cursor::Cursor<alloc::vec::Vec<u8>>>>>::decode_image_stream (1x)
        .                               predictor_data,
        9 ( 0.00%)                  } => apply_predictor_transform(
8,168,767 (19.35%)  => /home/runner/work/image-webp/image-webp/src/lossless/decoder/reverse_transform.rs:image_webp::lossless::decoder::reverse_transform::apply_predictor_transform (1x)
        .                               predictor_data,
        5 ( 0.00%)                      apply_color_transform(
6,556,274 (15.53%)  => /home/runner/work/image-webp/image-webp/src/lossless/decoder/reverse_transform.rs:image_webp::lossless::decoder::reverse_transform::apply_color_transform (1x)
...
2,710,990 ( 6.42%)  => src/lossless/decoder/mod.rs:<image_webp::lossless::decoder::LosslessDecoder<core::io::util::Take<&mut core::io::cursor::Cursor<alloc::vec::Vec<u8>>>>>::read_huffman_codes (3x)
   ...
   95,008 ( 0.23%)  => src/lossless/decoder/mod.rs:<image_webp::lossless::decoder::LosslessDecoder<core::io::util::Take<&mut core::io::cursor::Cursor<alloc::vec::Vec<u8>>>>>::decode_image_stream (1x)
        .                                   predictor_data,
       10 ( 0.00%)                      self.decode_image_stream(block_xsize, block_ysize, false, &mut transform_data)?;
  174,333 ( 0.41%)  => src/lossless/decoder/mod.rs:<image_webp::lossless::decoder::LosslessDecoder<core::io::util::Take<&mut core::io::cursor::Cursor<alloc::vec::Vec<u8>>>>>::decode_image_stream (1x)
       36 ( 0.00%)      fn read_huffman_codes(
        .                   color_cache: Option<ColorCache>,
        8 ( 0.00%)              self.decode_image_stream(huffman_xsize, huffman_ysize, false, &mut data)?;
   97,827 ( 0.23%)  => src/lossless/decoder/mod.rs:<image_webp::lossless::decoder::LosslessDecoder<core::io::util::Take<&mut core::io::cursor::Cursor<alloc::vec::Vec<u8>>>>>::decode_image_stream'2 (1x)
        .                               if let Some(color_cache) = color_cache.as_ref() {
        .                                   alphabet_size += 1 << color_cache.color_cache_bits;
    1,755 ( 0.00%)                  specs.push(self.read_huffman_code_spec(alphabet_size)?);
        .                       color_cache,
        .               fn read_huffman_code_spec(
    1,612 ( 0.00%)                  self.read_huffman_code_lengths(code_length_code_lengths, alphabet_size)?;
        .               fn read_huffman_code_lengths(
    1,736 ( 0.00%)          let table = HuffmanTree9::build_implicit(code_length_code_lengths)?;
  242,655 ( 0.57%)  => /home/runner/work/image-webp/image-webp/src/lossless/decoder/huffman.rs:<image_webp::lossless::decoder::huffman::HuffmanTree<9>>::build_implicit (123x)
        ...
        .                               color_cache,
        .                               color_cache,
   52,837 ( 0.13%)  => /rustc/88d9e12ae178fab0fb5cc050a94da85685d449ea/library/core/src/ptr/mod.rs:core::ptr::drop_glue::<image_webp::lossless::decoder::HuffmanInfo> (3x)
  ...
  284,038 ( 0.67%)          let index = (0x1e35a7bdu32.wrapping_mul(color_u32)) >> (32 - self.color_cache_bits);
  604,405 ( 1.43%)          self.color_cache[index as usize] = color;
    4,570 ( 0.01%)          self.color_cache[index]
        .               pub(crate) fn fill(&mut self) -> Result<(), DecodingError> {
        .                   let mut buf = self.reader.fill_buf()?;
  394,359 ( 0.93%)              self.reader.consume(usize::from((63 - self.nbits) / 8));
        .                           self.reader.consume(1);
        .                           buf = self.reader.fill_buf()?;
        .               pub(crate) fn consume(&mut self, num: u8) -> Result<(), DecodingError> {
        .                       self.fill()?;
        .                   self.consume(num)?;
1,239,818 ( 2.94%)  <counts for unidentified lines in src/lossless/decoder/mod.rs>
```
