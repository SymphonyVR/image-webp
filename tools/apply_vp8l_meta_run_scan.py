...
old_mask='''        let huffman_mask = if huffman_bits == 0 {\n            !0\n        } else {\n            (1 << huffman_bits) - 1\n        };\n\n'''
if old_mask not in s: raise SystemExit('huffman mask construction not found')
s=s.replace(old_mask,'',1)
old_init='''            bits: huffman_bits,\n            mask: huffman_mask,\n            huffman_code_groups: hufftree_groups,\n'''
new_init='''            bits: huffman_bits,\n            huffman_code_groups: hufftree_groups,\n'''
if old_init not in s: raise SystemExit('huffman mask init not found')
s=s.replace(old_init,new_init,1)
old_field='''    bits: u8,\n    mask: u16,\n    huffman_code_groups: Vec<HuffmanCodeGroup>,\n'''
new_field='''    bits: u8,\n    huffman_code_groups: Vec<HuffmanCodeGroup>,\n'''
if old_field not in s: raise SystemExit('huffman mask field not found')
