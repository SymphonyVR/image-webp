...
BASE='4f322d44fb38747659451db3d7f1dac7ff8ff21f';TMP=Path('/tmp/vp8l-color-unroll');VS=('base','u2','u4','u8')
...
def chunks(p):
 ...
 return o
def patch(r,n):
 p=r/'src/lossless/decoder/reverse_transform.rs';s=p.read_text();old='''            for pixel in block.chunks_exact_mut(4) {\n                inverse_color_pixel_packed(pixel, red_to_blue, green_to_blue, green_to_red);\n            }'''
 calls='\n'.join(f'                inverse_color_pixel_packed(&mut pixels[{i*4}..{i*4+4}], red_to_blue, green_to_blue, green_to_red);' for i in range(n))
 new=f'''            let mut pixel_groups = block.chunks_exact_mut({n*4});\n            for pixels in &mut pixel_groups {{\n{calls}\n            }}\n            for pixel in pixel_groups.into_remainder().chunks_exact_mut(4) {{\n                inverse_color_pixel_packed(pixel, red_to_blue, green_to_blue, green_to_red);\n            }}'''
...
def main():
 ...
 for v in VS:
  ...
  (r/'examples').mkdir(exist_ok=True);(r/'examples/cunroll.rs').write_text(BENCH);run(['cargo','build','--release','--example','cunroll','-q'],cwd=r,env=env);exes[v]=r/'target/release/examples/cunroll'
 ...
 for name,files,it in [('corpus',corpus,70),('colorhot',hot,45)]:
  ...
  for n in range(17):
   ...
   rows.append(z)
  results[name]=rows
 cpu=run(['bash','-lc',"lscpu|sed -n 's/^Model name:[[:space:]]*//p'"],cap=True);L=['# VP8L color-transform manual-unroll current-tree matrix','',f'- baseline: `{BASE}`',f'- CPU: `{cpu}`','- manually exposes 2/4/8 independent pixels per transform block to LLVM; hashes + full verification passed','','| workload | candidate | paired median | positive | range |','|---|---|---:|---:|---:|']
 ...
 Path('benchmark-vp8l-color-unroll-current.md').write_text('\n'.join(L)+'\n');print('\n'.join(L))
