...
BASE='4f322d44fb38747659451db3d7f1dac7ff8ff21f';TMP=Path('/tmp/vp8l-predictor1-word-current');VS=('base','word','word4')
...
fn add_pixel_lanes(a: u32, b: u32) -> u32 {
    let lo = (a & 0x00ff_00ff).wrapping_add(b & 0x00ff_00ff) & 0x00ff_00ff;
    let hi = (a & 0xff00_ff00).wrapping_add(b & 0xff00_ff00) & 0xff00_ff00;
    lo | hi
}

'''
WORD=r'''pub fn apply_predictor_transform_1(image_data: &mut [u8], range: Range<usize>, _width: usize) {
    ...
    let mut previous = u32::from_le_bytes(image_data[range.start - 4..range.start].try_into().unwrap());
    for pixel in image_data[range].chunks_exact_mut(4) {
        let residual = u32::from_le_bytes(pixel.try_into().unwrap());
        previous = add_pixel_lanes(residual, previous);
        pixel.copy_from_slice(&previous.to_le_bytes());
    }
}
'''
WORD4=r'''pub fn apply_predictor_transform_1(image_data: &mut [u8], range: Range<usize>, _width: usize) {
    assert!(range.end <= image_data.len());
    if range.start % 4 != 0 || (range.end - range.start) % 4 != 0 {
        let mut i = range.start;
        while i < range.end {
    ...
    let mut previous = u32::from_le_bytes(image_data[range.start - 4..range.start].try_into().unwrap());
    let mut chunks = image_data[range].chunks_exact_mut(16);
    for chunk in &mut chunks {
        for off in [0usize,4,8,12] {
            let residual = u32::from_le_bytes(chunk[off..off+4].try_into().unwrap());
            previous = add_pixel_lanes(residual, previous);
            chunk[off..off+4].copy_from_slice(&previous.to_le_bytes());
        }
    }
...
def chunks(p):
 ...
 return o
def patch(r,v):
 p=r/'src/lossless/decoder/reverse_transform.rs';s=p.read_text();anchor='pub fn apply_predictor_transform_1(';a=s.index(anchor);b=s.index('pub fn apply_predictor_transform_2(',a);s=s[:a]+(WORD if v=='word' else WORD4)+s[b:];insert=s.index('pub fn apply_predictor_transform_0(');s=s[:insert]+HELP+s[insert:];p.write_text(s)
...
def main():
 ...
 for v in VS:
  ...
  (r/'examples').mkdir(exist_ok=True);(r/'examples/p1word.rs').write_text(BENCH);run(['cargo','build','--release','--example','p1word','-q'],cwd=r,env=env);exes[v]=r/'target/release/examples/p1word'
 ...
 for name,files,it in [('corpus',corpus,70),('large',{v:[large]for v in VS},4)]:
  ...
  for n in range(17):
   ...
   rows.append(z)
  results[name]=rows
 cpu=run(['bash','-lc',"lscpu|sed -n 's/^Model name:[[:space:]]*//p'"],cap=True);L=['# VP8L predictor1 word-lane current-tree matrix','',f'- baseline: `{BASE}`',f'- CPU: `{cpu}`','- safe 32-bit independent byte-lane arithmetic; hashes + full verification passed','','| workload | candidate | paired median | positive | range |','|---|---|---:|---:|---:|']
 ...
 Path('benchmark-vp8l-predictor1-word-current.md').write_text('\n'.join(L)+'\n');print('\n'.join(L))
