...
BASE='84d8d20753fce0a9972e8a244fdf929b5a55671c';TMP=Path('/tmp/vp8l-secondary-reserve-confirm');VS=('base','r64','r128')
...
def main():
 ...
 for name,files,it in [('corpus',corpus,70),('large',{v:[large]for v in VS},4)]:
  ...
  for n in range(25):
   ...
   rows.append(z)
  results[name]=rows
 cpu=run(['bash','-lc',"lscpu|sed -n 's/^Model name:[[:space:]]*//p'"],cap=True);L=['# VP8L secondary Huffman reserve long confirmation','',f'- baseline: `{BASE}`',f'- CPU: `{cpu}`','- hashes + tests + release tests + docs + Clippy + fmt + MSRV debug/release passed','','| workload | candidate | paired median | positive | range |','|---|---|---:|---:|---:|']
 ...
 Path('benchmark-vp8l-secondary-reserve-confirm.md').write_text('\n'.join(L)+'\n');print('\n'.join(L))
